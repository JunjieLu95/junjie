...
from junjie.num.graph.examples import tetra_marburg_Wchange_2ports

k = np.linspace(0.01, 2000, 20000)+1j*1

E=k/(np.pi/7.244)

# vn_eff, vna_eff = tetra_marburg_Wchange_2ports(w=1)
# S = vna_eff.calcS(k)
# h = create_h(vn_eff,k)
...
print(1-np.abs(np.mean(S[:,[0,3]]))**2)
#%%
exp_S_cpa = np.reshape(S, (S.shape[0], 2, 2), order='F')
inv_S_cpa = np.linalg.inv(exp_S_cpa)[:, 0, 1] 

plt.plot(np.real(k), np.abs(inv_S_cpa)**2,  label='Exp')
#%%
from mesopylib.extract.fftOwn import fft4Dat
frq = np.real(k*3/20/np.pi)
t, fft_S = fft4Dat(frq, np.conj(S[:,[1]]), frqtotime=True)
plt.plot(t, np.abs(fft_S)**2)
